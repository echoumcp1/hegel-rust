...
impl<T> Variables<T> {
    fn pool_generate(&self, consume: bool) -> i128 {
        match self.tc.backend().pool_generate(self.pool_id, consume) {
            Ok(id) => id,
            Err(_) => {
    ...
    pub fn add(&mut self, v: T) {
        let variable_id: i128 = match self.tc.backend().pool_add(self.pool_id) {
            Ok(id) => id,
            Err(_) => {
...
pub fn variables<T>(tc: &TestCase) -> Variables<T> {
    let pool_id = match tc.backend().new_pool() {
        Ok(id) => id,
        Err(_) => {
            panic!("{}", STOP_TEST_STRING); // nocov
        }
